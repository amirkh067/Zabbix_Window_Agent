...
# Fetch the IP of the current machine
$ipAddress = (Get-NetIPAddress -AddressFamily IPv4).IPAddress
Write-Host "IP Address: $ipAddress"

# Fetch the proxy name specific to this IP from proxy.txt
...
if (-not $proxy) {
    Write-Host "Proxy value for IP $ipAddress not found in proxy.txt."
    exit 1
}
